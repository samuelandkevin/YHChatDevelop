...
//    }];
//    
//    self.navigationItem.rightBarButtonItem = [UIBarButtonItem rightItemWithImgName:@"" target:self selector:@selector(onRight:)];
    ...
    //设置WebScoket
    NSString *chatUserId = self.model.sessionUserId;
    [[YHChatManager sharedInstance] connectToUserID:chatUserId isGroupChat:self.model.isGroupChat];
...
- (void)withDrawMsg:(YHChatModel *)msg inRightCell:(CellChatTextRight *)rightCell{
    DDLog(@"撤回消息:\n%@",msg);
    if (![_chatHelper msgCanWithdraw:msg]){
...
- (void)didFinishPickingPhotosWithPhotos:(NSArray<UIImage *> *)photos{
    ...
    [self _uploadImage:compressImage];
}
...
- (void)_uploadImage:(UIImage *)compressImage{
    WeakSelf
    [[NetManager sharedInstance] postUploadImage:compressImage complete:^(BOOL success, id obj) {
        ...
        if (success){
            ...
            YHChatModel *model = [YHChatHelper creatMessage:oriUrlStr msgType:YHMessageType_Image toID:_model.sessionUserId];
            ...
            //发送图片消息
            int chatType = _model.isGroupChat?QChatType_Group:QChatType_Private;
            [[NetManager sharedInstance] postSendChatMsgToReceiverID:weakSelf.model.sessionUserId msgType:YHMessageType_Image msg:model.msgContent chatType:chatType complete:^(BOOL success, id obj) {
                if (success) {
                    DDLog(@"图片消息发送成功:%@",obj);
                }else{
                    DDLog(@"图片消息发送失败:%@",obj);
