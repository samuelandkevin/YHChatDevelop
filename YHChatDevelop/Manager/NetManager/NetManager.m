    ...
    YHURLSessionTask *session = [self.requestManager GET:requestUrl parameters:parameters progress:^(NSProgress * _Nonnull uploadProgress) {
        progress(uploadProgress);
         ...
         success:^(NSURLSessionDataTask * _Nonnull task, id  _Nullable responseObject) {
             ...
             if([self canParseResponseObject:responseObject jsonObj:&jsonObj requestUrl:requestUrl]){
         ...
         progress:^(NSProgress * _Nonnull uploadProgress)
     {
         ...
         if (progress) {
          ...
          success:^(NSURLSessionDataTask * _Nonnull task, id  _Nullable responseObject) {
              ...
              NSDictionary *jsonObj = nil;
              if([self canParseResponseObject:responseObject jsonObj:&jsonObj requestUrl:url]){
...
- (BOOL)canParseResponseObject:(NSData *)responseObject jsonObj:(NSDictionary **)jsonObj requestUrl:(NSString *)requestUrl{
    ...
    if (parseJsonError) {
        DDLog(@"json 解析失败:%@\nrequestUrl:%@",parseJsonError.localizedDescription,requestUrl);
        return NO;
    }
    else {
