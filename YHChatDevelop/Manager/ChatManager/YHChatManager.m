...
@property (nonatomic,copy)   void(^newMsgBlock)(YHChatModel *);
@end
...
- (void)connectToUserID:(NSString *)toUserId isGroupChat:(BOOL)isGroupChat{
    ...
    //is_group 是否群聊0否 1是
    NSString *token   = [YHUserInfoManager sharedInstance].userInfo.accessToken;
...
- (void)webSocket:(SRWebSocket *)webSocket didReceiveMessage:(id)message{
    if ([message isKindOfClass:[NSString class]]) {
        ...
        YHChatModel *model = [[DataParser shareInstance] parseOneChatLogWithDict:retDict];
        if (_newMsgBlock) {
            _newMsgBlock(model);
        }
        DDLog(@"收到数据了:%@",model);
