...
- (void)setDictConfig:(NSDictionary *)dictConfig{
    ...
    itemBgColor = itemBgColor?itemBgColor:kBlueColor;
...
- (void)drawRect:(CGRect)rect{
    ...
    [bezierPath addLineToPoint:CGPointMake(5, 0)];
    [bezierPath addLineToPoint:CGPointMake(10, 5)];
    [bezierPath closePath];
    
    UIColor *fillColor = kBlueColor;
    [fillColor set];
    [bezierPath fill];
...
@property (nonatomic,strong) YHArrow *viewArrow;
...
- (void)setup{
    ...
    YHArrow *viewArrow = [[YHArrow alloc] initWithFrame:CGRectZero];
    viewArrow.backgroundColor = [UIColor clearColor];
    [self addSubview:viewArrow];
    _viewArrow = viewArrow;
...
- (void)layoutUI{
    ...
    [_viewArrow mas_makeConstraints:^(MASConstraintMaker *make) {
        make.left.equalTo(weakSelf).offset(weakSelf.menuViewX+_menuViewW-26);
        make.top.equalTo(weakSelf).offset(weakSelf.menuViewY-5);
...
- (NSDictionary *)config{
    if (!_config) {
        _itemBgColor = _itemBgColor?_itemBgColor:kBlueColor;
