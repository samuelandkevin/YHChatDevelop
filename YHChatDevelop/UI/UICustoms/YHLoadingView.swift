...
//
//  Created by samuelandkevin on 2017/5/27.
//  Copyright © 2017年 samuelandkevin. All rights reserved.
//

import Foundation

class YHLoadingView: UIView,CAAnimationDelegate {

    var round1Color = kBlueColor_Swift
    var round2Color = kBlueColor_Swift.withAlphaComponent(0.6)
    var round3Color = kBlueColor_Swift.withAlphaComponent(0.3)
    ...
    //显示加载动画在指定的view上
    class func showLoadingIn(view:UIView) -> YHLoadingView
    {
        let loadingView = YHLoadingView.init(frame: CGRect.init(x: 0, y: 0, width: view.width, height: view.height))
    ...
    //显示加载动画在指定的window上
    class func showLoadingInWindow()->YHLoadingView
    {
        let lastWindow = UIApplication.shared.windows.last
        
        let loadingView = YHLoadingView.init(frame: CGRect.init(x: 0, y: 0, width: UIScreen.main.bounds.width, height: UIScreen.main.bounds.height))
