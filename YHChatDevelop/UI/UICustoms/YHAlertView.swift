...
class YHAlertView : UIView{
    ...
    /** 3-1.图标 */
    private var _imgvIcon:UIImageView?
    /** 3-2.标题视图 */
    ...
    public convenience init(icon:String,message:String?,delegate:YHAlertViewDelegate?,cancelButtonTitle:String?,otherButtonTitles:String ...){
        self.init()
        
        _arrayButton    = [UIButton]()
        
        //图标
        _imgvIcon       = UIImageView(image: UIImage(named:icon))
        if let imgv = _imgvIcon{
    ...
    //显示图标+msg的弹框
    open class func show(icon:String,message:String?,cancelButtonTitle:String?,otherButtonTitle:String,clickButtonBlock:YHAlertViewClickButtonBlock){
        let alertView = YHAlertView(icon: icon, message: message, delegate: nil, cancelButtonTitle: cancelButtonTitle, otherButtonTitles: otherButtonTitle)
        alertView._clickButtonBlock = clickButtonBlock
        alertView.show()
    }
    ...
    private func _setupButton(){
        ...
        switch countRow {
            ...
            case 2:
                ...
                let buttonCancel = _button(frame:  CGRect(x: 0, y: buttonY, width: _contentWidth/2, height: _contentHeight/2), title: titleCancel, target: self, action: #selector(_clickCancel(sender:)))
    ...
    fileprivate func _layoutsubviews(){
        ...
        if let imgvIcon = _imgvIcon {
            //图标frame
            imgvIcon.frame = CGRect(x: (_contentWidth-iconW)/2, y: 20, width: iconW, height: iconW)
            //消息frame
            _labelMessage.frame = CGRect(x: labelX, y: imgvIcon.frame.maxY+15, width: labelW, height: sizeMessage.height)
        }else{
            //消息frame
    ...
    // MARK: - Life 
    deinit {
        debugPrint("YHAlertView is dealloc")
    }
}
