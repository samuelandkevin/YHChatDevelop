...
+ (YHChatModel *)creatMessage:(NSString *)msg msgType:(YHMessageType)msgType  toID:(NSString *)toID {
    ...
    model.speakerId     = [YHUserInfoManager sharedInstance].userInfo.uid;
    model.speakerAvatar = [YHUserInfoManager sharedInstance].userInfo.avatarUrl;
    model.speakerName   = [YHUserInfoManager sharedInstance].userInfo.userName;
    model.direction     = 0;
...
- (void)registerCellClassWithTableView:(UITableView *)tableView{
    ...
    [tableView registerClass:[CellChatCheckinLeft class] forCellReuseIdentifier:NSStringFromClass([CellChatCheckinLeft class])];
    [tableView registerClass:[CellChatCheckinRight class] forCellReuseIdentifier:NSStringFromClass([CellChatCheckinRight class])];
...
- (CGFloat)heightWithModel:(YHChatModel *)model tableView:(UITableView *)tableView{
    ...
    if(model.status == 1){
        ...
        if (model.msgType == YHMessageType_Image) {
            ...
            if (model.direction == 0) {
                height = [CellChatCheckinRight hyb_heightForTableView:tableView config:^(UITableViewCell *sourceCell) {
                    CellChatCheckinRight *cell = (CellChatCheckinRight *)sourceCell;
                    [cell setupModel:model];
                }];
            }else{
                height = [CellChatCheckinLeft hyb_heightForTableView:tableView config:^(UITableViewCell *sourceCell) {
                    CellChatCheckinLeft *cell = (CellChatCheckinLeft *)sourceCell;
                    [cell setupModel:model];
                }];
            }
...
//消息是否可以撤回
- (BOOL)msgCanWithdraw:(YHChatModel *)model{
    if (model.createTime) {
        NSDate *createDate = [NSDate dateByStringFormat:@"yyyy-MM-dd HH:mm:ss" dateString:model.createTime];
