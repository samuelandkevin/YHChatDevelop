...
@interface CellForWorkGroup()<YHWorkGroupBottomViewDelegate>
...
//    self.imgvAvatar.layer.masksToBounds = YES;
    ...
    self.labelContent.font = [UIFont systemFontOfSize: (contentLabelFontSize+self.addFontSize)];
    ...
    _labelContent.highlightTapAction = ^(UIView *containerView, NSAttributedString *text, NSRange range, CGRect rect) {
        ...
        if (weakSelf.delegate && [weakSelf.delegate respondsToSelector:@selector(onLinkInCell:linkType:linkText:)]) {
            [weakSelf.delegate onLinkInCell:weakSelf linkType:linkType linkText:linkText];
        }
