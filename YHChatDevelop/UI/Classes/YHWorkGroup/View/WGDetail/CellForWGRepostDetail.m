...
@interface CellForWGRepostDetail()
...
@implementation CellForWGRepostDetail
...
//    self.imgvAvatar.layer.masksToBounds = YES;
    ...
    _labelContent.highlightTapAction = ^(UIView *containerView, NSAttributedString *text, NSRange range, CGRect rect) {
        ...
        if (weakSelf.delegate && [weakSelf.delegate respondsToSelector:@selector(onLinkInWGRepostDetailCell:linkType:linkText:)]) {
            [weakSelf.delegate onLinkInWGRepostDetailCell:weakSelf linkType:linkType linkText:linkText];
        }
    };
    ...
    UITapGestureRecognizer *tapRepost =[[UITapGestureRecognizer alloc] initWithTarget:self action:@selector(tapRepostView:)];
    [_repostView addGestureRecognizer:tapRepost];
...
//        make.top.equalTo(weakSelf.repostView.mas_bottom);
...
- (void)setModel:(YHWorkGroup *)model{
    ...
    [_repostView mas_remakeConstraints:^(MASConstraintMaker *make) {
...
- (void)onAvatar:(UITapGestureRecognizer *)recognizer{
    ...
    if(recognizer.state == UIGestureRecognizerStateEnded){
        if (_delegate && [_delegate respondsToSelector:@selector(onAvatarInWGRepostDetailCell:)]) {
            [_delegate onAvatarInWGRepostDetailCell:self];
        }
    }
}
...
- (void)tapRepostView:(UIGestureRecognizer *)guesture{
    if (guesture.state == UIGestureRecognizerStateEnded) {
        if (_delegate && [_delegate respondsToSelector:@selector(onTapRepostViewInWGRepostDetailCell:)]) {
            [_delegate onTapRepostViewInWGRepostDetailCell:self];
        }
    }
}
...
- (void)onMoreTap
{
    ...
    if (_delegate && [_delegate respondsToSelector:@selector(onMoreInWGRepostDetailCell:)]) {
        [_delegate onMoreInWGRepostDetailCell:self];
    }
}

- (void)deleteTap{
    
    if (_delegate && [_delegate respondsToSelector:@selector(onDeleteInWGRepostDetailCell:)]) {
        [_delegate onDeleteInWGRepostDetailCell:self];
    }
}
