...
- (NSCachedURLResponse *)localCacheResponeWithRequest:(NSURLRequest *)request {
    ...
    if ([fm fileExistsAtPath:filePath]) {
        ...
        if (expire == false) {
            ...
            /***这里耦合度有点高,是根据服务器返回的成功或失败信息将缓存清除***/
            NSString *dataStr = [[NSString alloc] initWithData:data encoding:NSUTF8StringEncoding];
            NSString *errorMsg = @"{\"status\"";
            if ([dataStr hasPrefix:errorMsg]) {
                [fm removeItemAtPath:filePath error:nil];      //清除缓存data
                [fm removeItemAtPath:otherInfoPath error:nil]; //清除缓存其它
                return nil;
            }
...
-(NSMutableDictionary *)blackListsRequestUrl{
    if (!_blackListsRequestUrl) {
        _blackListsRequestUrl = [NSMutableDictionary dictionary];
    }
    return _blackListsRequestUrl;
}
