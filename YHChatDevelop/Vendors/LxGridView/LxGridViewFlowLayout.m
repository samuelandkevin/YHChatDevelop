...
- (NSArray *)layoutAttributesForElementsInRect:(CGRect)rect
{
    ...
    //从第二个循环到最后一个
    for(int i = 1; i < [layoutAttributesForElementsInRect count]; ++i) {
        //当前attributes
        UICollectionViewLayoutAttributes *currentLayoutAttributes = layoutAttributesForElementsInRect[i];
        //上一个attributes
        UICollectionViewLayoutAttributes *prevLayoutAttributes = layoutAttributesForElementsInRect[i-1];
        //我们想设置的最大间距，可根据需要改
        ...
        if(origin + maximumSpacing + currentLayoutAttributes.frame.size.width + 10 < self.collectionViewContentSize.width) {
