...
#define kDefaultBGColor kTbvBGColor
...
- (void)layoutSubviews{
    self.contentView.frame = CGRectMake(0.0, CGRectGetMaxY(self.segmentView.frame)+self.contentViewTop, self.bounds.size.width, self.bounds.size.height - CGRectGetMaxY(self.segmentView.frame));
    
    self.backgroundColor = self.bgColor? self.bgColor:kDefaultBGColor;
    
}

- (void)dealloc {
    DDLog(@"ZJScrollPageView--销毁");
}
