...
//    DDLog(@"old ---- %ld current --- %ld", _oldIndex, _currentIndex);
...
- (void)setupChildVcForCell:(UICollectionViewCell *)cell atIndexPath:(NSIndexPath *)indexPath {
    ...
    //    DDLog(@"当前的index:%ld", indexPath.row);
    if (_isLoadFirstView) {
...
//    DDLog(@"出现出现:current:---- %ld   old ----- %ld indexpathRow----%ld ", _currentIndex, _oldIndex, indexPath.row);
    if (_sysVersion >= 8) {
        [self setupChildVcForCell:cell atIndexPath:indexPath];
    }
...
//    DDLog(@"消失消失:current:---- %ld   old ----- %ld indexpathRow----%ld ", _currentIndex, _oldIndex, indexPath.row);
