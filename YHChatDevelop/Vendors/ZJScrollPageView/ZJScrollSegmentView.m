...
//        [self setupTitles];
        ...
        //默认显示选中第一个
        ZJTitleView *firstTitleView = (ZJTitleView *)self.titleViews[0];
        firstTitleView.currentTransformSx = 1.0;
        if (firstTitleView) {
            
            // 缩放, 设置初始的label的transform
            if (self.segmentStyle.isScaleTitle) {
                firstTitleView.currentTransformSx = self.segmentStyle.titleBigScale;
            }
            // 设置初始状态文字的颜色
            firstTitleView.textColor = self.segmentStyle.selectedTitleColor;
...
- (void)setupTitles {
    ...
    for (NSString *title in self.titles) {
        ...
        //kunModify
        [self setupfontSizeWithLabel:titleView.label];
...
//        CGFloat titleViewWidth = [titleView titleViewWidth];
        ...
        //kunModify
        CGRect bounds = [title boundingRectWithSize:CGSizeMake(MAXFLOAT, 0.0) options:NSStringDrawingUsesLineFragmentOrigin attributes:@{NSFontAttributeName: titleView.label.font} context:nil];
...
//设置label字体大小
- (void)setupfontSizeWithLabel:(UILabel *)label{
    CGFloat addFontSize = [[[NSUserDefaults standardUserDefaults] valueForKey:kSetSystemFontSize] floatValue];
    
    label.font = [UIFont systemFontOfSize:self.segmentStyle.titleFont.pointSize + addFontSize];
}
